...
void JobExecutor::DoWork() {
  while (true) {
    ...
    // wait until we're shutting down or there are some jobs to run
    jobs_changed_cv_.wait(
        lk, [&]() { return shutting_down_ || !scheduled_jobs_.empty(); });
    if (shutting_down_) {
      break;
    }
    assert(!scheduled_jobs_.empty());
    auto earliest_job_time = scheduled_jobs_.begin()->first;
    // wait until either:
    // 1) the next job should be run
    // 2) we are shutting down
    // 3) the job was scheduled that needs to be executed earlier than the
    // previous earliest job
    jobs_changed_cv_.wait_until(lk, earliest_job_time, [&] {
      return shutting_down_ || scheduled_jobs_.empty() ||
             earliest_job_time != scheduled_jobs_.begin()->first;
    });
    if (shutting_down_) {
      break;
    }
    ...
    auto job = scheduled_jobs_.begin();
    if (job->first <= std::chrono::steady_clock::now()) {
      // invoke the function
      lk.unlock();
      job->second();
      lk.lock();
      scheduled_jobs_.erase(job);
    }
  }
}
